...
import { users, userRoles, studentProgress, lessonContent, practicalMaterials } from '../db/schema.js';
...
export async function adminRoutes(app: FastifyInstance) {
  ...
  // Admin: set user role
  app.post<{ Body: { userId: string; role: 'admin' | 'student' } }>('/admin/set-role', async (req, reply) => {
    const payload = getAuthFromRequest(req);
    if (!payload || payload.role !== 'admin') {
      return reply.status(403).send({ error: 'Требуются права администратора' });
    }
    ...
    if (!userId || !role || !['admin', 'student'].includes(role)) {
      return reply.status(400).send({ error: 'userId и role (admin|student) обязательны' });
    }
    try {
