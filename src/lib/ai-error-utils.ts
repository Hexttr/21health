...
export function translateAiErrorMessage(message: string): string {
...
export function normalizeAiErrorMessage(error: unknown, fallback = 'Произошла ошибка AI-сервиса. Попробуйте еще раз.'): string {
  if (error instanceof Error && error.message.trim()) {
    return translateAiErrorMessage(error.message);
  }
  if (typeof error === 'string' && error.trim()) {
    return translateAiErrorMessage(error);
  }
  return fallback;
}

export function showPersistentAiError(error: unknown, fallback?: string): void {
  toast.error(normalizeAiErrorMessage(error, fallback), {
    duration: Infinity,
    closeButton: true,
  });
}
